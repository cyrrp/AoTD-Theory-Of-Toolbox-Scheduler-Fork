...
public class AoTDGrandWonderBtnListener implements IndustryOptionProvider {
    ...
    public List<IndustryOptionData> getIndustryOptions(Industry ind) {
        if(ind.getId().contains("aotd_gw_construction_site")){
            ...
            if(!ind.isUpgrading()){
                IndustryOptionData data = new IndustryOptionData("Choose Wonder to Build",WONDER,ind,this);
        ...
        if(ind instanceof GrandWonderAPI){
            if(!ind.canShutDown()&&!ind.showShutDown()){
                IndustryOptionData data = new IndustryOptionData("Dismantle Wonder",REMOVE_WONDER,ind,this);
    ...
    public void createTooltip(IndustryOptionData opt, TooltipMakerAPI tooltip, float width) {
        if(opt.id.equals(WONDER)){
            ...
            tooltip.addPara("Warning! This action might have greater consequences across entire faction!",Misc.getNegativeHighlightColor(),5f);
        }
    ...
    public void optionSelected(IndustryOptionData opt, DialogCreatorUI ui) {
        if(opt.id.equals(WONDER)){
            AshMisc.initPopUpDialog(new GrandWonderChooseConstructionDialog(ui, (AoTDConstructionSite) opt.ind),1000,650);
        }
        else{
