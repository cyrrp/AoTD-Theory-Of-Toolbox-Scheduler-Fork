...
public class AoTDExcDefData {
    ...
    public void recordDemandForThisMonth(AoTDCommodityOnMarket commodity) {
        this.recordedDemandFromNonPendingThisMonth =
    ...
    public void clearExternalTrade(AoTDCommodityOnMarket commodity) {
        ...
        if (excessFromTrade > 0) {
             ...
             */
            try {
                if (commodity.getMarket() != null
                        && commodity.getMarket().getMemoryWithoutUpdate() != null) {
                    commodity
                            .getMarket()
                            .getMemoryWithoutUpdate()
                            .set(
                                    EffectivePriceCalculator.LOCAL_PLAYER_TRADE_MEMORY_PREFIX
                                            + commodity.getId(),
                                    Math.max(
                                            1f,
                                            excessFromTrade
            ...
            if (commodity.getMarket().hasSubmarket(Submarkets.LOCAL_RESOURCES)) {
                if (commodity.getMarket().getSubmarket(Submarkets.LOCAL_RESOURCES).getCargo()
                        != null) {
                    commodity
                            .getMarket()
                            .getSubmarket(Submarkets.LOCAL_RESOURCES)
                            .getCargo()
                            .addCommodity(commodity.getId(), commodity.getExcessQuantity());
    ...
    public void applyExternalTrade(
            int deficitAmt, int excessAmt, float days, AoTDCommodityOnMarket com) {
        if (deficitAmt > 0)
            deficit.addTemporaryModFlat(
                    days, EXT_TRADE_ID, "Unable to import due to Global Deficit.", deficitAmt);
        ...
        if (excessAmt > 0) {
            if (com.getSpec().hasTag(AoTDTradeTags.AOTD_DOES_NOT_HAVE_EXCESS)) {
                ...
                if (com.getSpec().hasTag(AoTDTradeTags.AOTD_NO_ONE_BUYS_OUTSIDE)) {
                    excess.addTemporaryModFlat(
                            days,
                            EXT_TRADE_ID,
                            "Unable to export due to Global Excess.",
                            excessAmt);
                } else {
                    int soldMax = excessAmt / 2;
                    AoTDTradeManager.getInstance()
                            .getMarketData(com.getMarket())
                            .addExtraSold(com.getSpec().getId(), soldMax);
                    excess.addTemporaryModFlat(
                            days, EXT_TRADE_ID, "Unable to export due to Global Excess.", soldMax);
                }
            }
    ...
    /**
     * Builds the sudden-demand change without mutating the live commodity. The price pipeline
     * carries this proposal with the snapshot and only applies it after the work ticket has passed
    ...
    public PreparedSuddenDemandUpdate prepareDeficitDueToSuddenChangeOfDemand(
        ...
        int projectedEffectiveDeficit = Math.max(0, projectedDeficitRaw - excess.getModifiedInt());
        int projectedEffectiveExcess = Math.max(0, excess.getModifiedInt() - projectedDeficitRaw);
        ...
        return new PreparedSuddenDemandUpdate(
                ...
                nextModifier,
                replaceModifier,
                projectedEffectiveDeficit,
                projectedEffectiveExcess);
    }

    /** Applies a previously prepared change after generation validation. */
    public void commitPreparedSuddenDemandUpdate(PreparedSuddenDemandUpdate prepared) {
        ...
        if (prepared.modifierAmount > 0) {
            deficit.addTemporaryModFlat(
                    31, DEF_FROM_NEW_IND, "Sudden surge of demand", prepared.modifierAmount);
        } else {
            deficit.removeTemporaryMod(DEF_FROM_NEW_IND);
        }
    }
    ...
    public void applyDeficitDueToSuddenChangeOfDemand(AoTDCommodityOnMarket commodity) {
        commitPreparedSuddenDemandUpdate(prepareDeficitDueToSuddenChangeOfDemand(commodity));
    }

    private static int getFlatModifierInt(MutableStat stat, String modifierId) {
    ...
    public void setExcess(
            int excessAmount, AoTDCommodityOnMarket commodity, float days, String id) {
        this.excess.addTemporaryModFlat(
                days, id, "Unable to export due to Global Excess.", excessAmount);
    }
    ...
    public void setDeficit(
            int deficitAmount, AoTDCommodityOnMarket commodity, float days, String id) {
        this.deficit.addTemporaryModFlat(
                days, id, "Unable to import due to Global Deficit.", deficitAmount);
    }
