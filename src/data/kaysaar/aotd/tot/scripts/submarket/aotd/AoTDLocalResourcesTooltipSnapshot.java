...
/**
 * Builds a self-consistent, call-local snapshot for the Local Resources tooltip. Expensive virtual
 * limit calculations happen once per commodity and never from the sort comparator.
 */
public final class AoTDLocalResourcesTooltipSnapshot {
    ...
    private static final class Row {
        ...
        private Row(CommodityOnMarketAPI commodity, int rawLimit, int sourceOrder) {
    ...
    public static void render(
            LocalResourcesSubmarketPlugin plugin, TooltipMakerAPI tooltip, LimitResolver resolver) {
        ...
        for (CommodityOnMarketAPI commodity : commodities) {
            rows.add(new Row(commodity, resolver.getStockpileLimit(commodity), sourceOrder++));
        }

        rows.sort(
                Comparator.comparingInt((Row row) -> row.rawLimit)
                        .reversed()
        ...
        for (Row row : rows) {
            ...
            int monthlyRate =
                    (int) Math.round(row.rawLimit * plugin.getStockpilingAddRateMult(commodity));
        ...
        tooltip.addPara(
                "A portion of the resources produced by the colony will be made available here. "
                        ...
                        + "This cost will be deducted at the end of the month.",
                opad,
                Misc.getHighlightColor(),
                ""
                        + (int) Math.round(LocalResourcesSubmarketPlugin.STOCKPILE_COST_MULT * 100f)
                        + "%");
        ...
        tooltip.addPara(
                "These resources can also be used to counter temporary shortages, for a "
                        ...
                        + "will be used as well, at no cost.",
                opad,
                Misc.getHighlightColor(),
                ""
                        + (int)
                                Math.round(
                                        LocalResourcesSubmarketPlugin.STOCKPILE_SHORTAGE_COST_MULT
                                                * 100f)
                        + "%");
        ...
        if (rowIndex > 0) {
            ...
            tooltip.addPara(
                    "Stockpiles are limited to %s the monthly rate.",
                    opad,
                    Misc.getHighlightColor(),
                    "" + (int) LocalResourcesSubmarketPlugin.STOCKPILE_MAX_MONTHS + Strings.X);
        } else {
    ...
    /**
     * Returns a read-only AoTD limit from the last published supply/demand data, or null when the
     * caller must use its existing correctness fallback once.
     */
    public static Integer peekAoTDStockpileLimit(
            AoTDCommodityOnMarket commodity, Map<String, MutableStat> stockpilingBonus) {
