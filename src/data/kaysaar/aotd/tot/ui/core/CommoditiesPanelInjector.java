...
public class CommoditiesPanelInjector implements MarketUIListener {
    ...
    public void onMarketOverviewDiscovered(IndustryPanelContextUI ctx) {
                ...
                ReflectionUtilis.getChildrenCopy(
                        (UIPanelAPI)
                                ReflectionUtilis.invokeMethodWithAutoProjection(
                                        "getScreenPanel", state))) {
            if (ReflectionUtilis.hasMethodOfName("computeShutdownRefund", componentAPI)) {
                if (!replaced) {
                    for (UIComponentAPI uiComponentAPI :
                            ReflectionUtilis.getChildrenCopy((UIPanelAPI) componentAPI)) {
                        if (uiComponentAPI instanceof ButtonAPI bt) {
                            ...
                            if (label.getText().toLowerCase().contains("downgrade")) {
                                Industry ind =
                                        (Industry)
                                                ReflectionUtilis.findFieldOfClass(
                                                        componentAPI, Industry.class);

                                Industry downgrade =
                                        ind.getMarket()
                                                .instantiateIndustry(ind.getSpec().getDowngrade());
                                TooltipMakerAPI tl =
                                        Global.getSettings()
                                                .createCustom(0, 0, null)
                                                .createUIElement(0, 0, false);
                                TooltipMakerAPI.TooltipLocation location =
                                        TooltipMakerAPI.TooltipLocation.RIGHT;

                                if (uiComponentAPI.getPosition().getX()
                                                + uiComponentAPI.getPosition().getWidth()
                                                + 320
                                                + 400
                                ...
                                tl.addTooltipTo(
                                        new TooltipMakerAPI.TooltipCreator() {
                                            ...
                                            public void createTooltip(
                                                    ...
                                                    boolean expanded,
                                                    Object tooltipParam) {
                                                IndustryOnHoverTooltipV2 v2 =
                                                        new IndustryOnHoverTooltipV2(
        ...
        for (UIComponentAPI componentAPI : ReflectionUtilis.getChildrenCopy(mainPanel)) {
            ...
            if (ReflectionUtilis.hasMethodOfName("getShipping", componentAPI)) {
                UIComponentAPI shipping =
                        (UIComponentAPI)
                                ReflectionUtilis.invokeMethodWithAutoProjection(
                                        "getShipping", componentAPI);
        ...
        if (toRemove != null) {
            if (!ctx.grandColoniesLayout) {
                if (widgets != null) {
                    TooltipMakerAPI tl =
                            Global.getSettings()
                                    .createCustom(1, 1, null)
                                    .createUIElement(1, 1, false);
                    for (UIPanelAPI object :
                            new ArrayList<UIPanelAPI>(
                                    (Collection)
                                            ReflectionUtilis.invokeMethodWithAutoProjection(
                                                    "getWidgets", widgets))) {
                        final Industry ind =
                                (Industry)
                                        ReflectionUtilis.findFieldOfClass(object, Industry.class);
                        for (UIComponentAPI componentAPI :
                                ReflectionUtilis.getChildrenCopy(object)) {
                        ...
                        tl.addTooltipTo(
                                new TooltipMakerAPI.TooltipCreator() {
                                    ...
                                    public void createTooltip(
                                        ...
                                        tooltip.addCustom(v2.getMainPanel(), 0f);
                                        if (ind.getSpec() != null
                                                && ind.getSpec().hasTag(Tags.CODEX_UNLOCKABLE)) {
                                            SharedUnlockData.get()
                                                    .reportPlayerAwareOfIndustry(
                                                            ind.getSpec().getId(), true);
                                        }
                                        tooltip.setCodexEntryId(
                                                CodexDataV2.getIndustryEntryId(
                                                        ind.getSpec().getId()));
                                    }
                                },
                                object,
            ...
            if (incomeInjector != null) {
                UIComponentAPI imigr =
                        (UIComponentAPI)
                                ReflectionUtilis.invokeMethodWithAutoProjection(
                                        "getImmigration", incomeInjector);
                IncomePanel panel = new IncomePanel(market);
                HazardRatingPanel hazardRatingPanel = new HazardRatingPanel(market);
                incomeInjector
                        .addComponent(panel.getMainPanel())
                        .rightOfTop(imigr, 5 + panel.getMainPanel().getPosition().getWidth());
                incomeInjector
                        .addComponent(hazardRatingPanel.getMainPanel())
                        .belowMid(panel.getMainPanel(), 18);
            }
            TooltipMakerAPI tl =
                    Global.getSettings().createCustom(1, 1, null).createUIElement(1, 1, false);
            if (ctx.stabAccessPanel != null) {
                tl.addTooltipTo(
                        new AccessibilityOnHover(market),
                        ReflectionUtilis.getChildrenCopy(ctx.stabAccessPanel).get(1),
                        TooltipMakerAPI.TooltipLocation.BELOW,
                        false);
            }
            mainPanel.getPosition().inTL(-xAxis, 30);
            AoTDCommodityPanel newPanel =
                    new AoTDCommodityPanel(
                            toRemove.getPosition().getWidth() + xAxis + extraWidth,
                            toRemove.getPosition().getHeight() + additionalHeight,
                            market,
                            false);
            newPanel.setMainColonyPanel(ctx.mainColonyPanel);
            mainPanel.removeComponent(toRemove);
            mainPanel
                    .addComponent(newPanel.getMainPanel())
                    .inBR(
                            -xAxis - extraWidth,
                            mainPanel.getPosition().getHeight() - 610 + additionalHeight);
        }
    }
    ...
    private void injectForGrandColonies(UIPanelAPI panelOfIndustries, MarketAPI market) {
        CustomPanelAPI panelAPI =
                (CustomPanelAPI)
                        ReflectionUtilis.getChildrenCopy((UIPanelAPI) panelOfIndustries).stream()
                                .filter(
                                        x ->
                                                x instanceof CustomPanelAPI
                                                        && x.getPosition().getWidth() == 830
                                                        && x.getPosition().getHeight() == 400)
                                .findFirst()
                                .orElse(null);
        TooltipMakerAPI tl =
                Global.getSettings().createCustom(1, 1, null).createUIElement(1, 1, false);
        if (panelAPI != null) {
            UIPanelAPI panelInsider =
                    (UIPanelAPI) ReflectionUtilis.getChildrenCopy(panelAPI).get(0);
            if (ReflectionUtilis.hasMethodOfName("getContentContainer", panelInsider)) {
                Object container =
                        ReflectionUtilis.invokeMethodWithAutoProjection(
                                "getContentContainer", panelInsider);
                TooltipMakerAPI tooltip =
                        (TooltipMakerAPI)
                                ReflectionUtilis.getChildrenCopy((UIPanelAPI) container).get(0);
                UIPanelAPI contentInside =
                        (UIPanelAPI) ReflectionUtilis.getChildrenCopy(tooltip).get(0);
                ArrayList<CustomPanelAPI> insiders = new ArrayList<>();
                for (UIComponentAPI uiComponentAPI :
                        ReflectionUtilis.getChildrenCopy(contentInside)) {
                ...
                for (CustomPanelAPI insider : insiders) {
                    ...
                    for (UIComponentAPI componentAPI :
                            ReflectionUtilis.getChildrenCopy((UIPanelAPI) widget)) {
                        if (componentAPI instanceof IconGroupAPI) {
                    ...
                    Industry ind =
                            (Industry) ReflectionUtilis.findFieldOfClass(widget, Industry.class);
                    ...
                    tl.addTooltipTo(
                            new TooltipMakerAPI.TooltipCreator() {
                                ...
                                public void createTooltip(
                                    ...
                                    IndustryOnHoverTooltipV2 v2 =
                                            new IndustryOnHoverTooltipV2(
                                                    getTooltipWidth(tooltipParam), ind, expanded);
                                    tooltip.addCustom(v2.getMainPanel(), 0f);
                                    if (ind.getSpec() != null
                                            && ind.getSpec().hasTag(Tags.CODEX_UNLOCKABLE)) {
                                        SharedUnlockData.get()
                                    ...
                                    tooltip.setCodexEntryId(
                                            CodexDataV2.getIndustryEntryId(ind.getSpec().getId()));
                                }
                            },
                            widget,
                            TooltipMakerAPI.TooltipLocation.RIGHT,
                ...
                UIPanelAPI contentInside =
                        (UIPanelAPI) ReflectionUtilis.getChildrenCopy(panelInsider).get(0);
                ArrayList<CustomPanelAPI> insiders = new ArrayList<>();
                for (UIComponentAPI uiComponentAPI :
                        ReflectionUtilis.getChildrenCopy(contentInside)) {
                    if (uiComponentAPI instanceof CustomPanelAPI panel) {
