...
public class AoTDToolboxTheoryPlugin extends BaseModPlugin
        implements MarketContextListenerInjector {
    ...
    public static void loadMarketsToReplace() {
        if (Global.getSettings().getModManager().isModEnabled("nexerelin")) {
            marketsToReplace.put(
                    Nex_OpenMarketPlugin.class.getName(), AoTDxNexOpenMarketPlugin.class.getName());
            marketsToReplace.put(
                    Nex_BlackMarketPlugin.class.getName(),
                    AoTDxNexBlackMarketPlugin.class.getName());
            marketsToReplace.put(
                    Nex_MilitarySubmarketPlugin.class.getName(),
                    AoTDxNexMilitarySubmarketPlugin.class.getName());
            marketsToReplace.put(
                    Nex_LocalResourcesSubmarketPlugin.class.getName(),
                    AoTDxNexLocalResourcesSubmarketPlugin.class.getName());
        }
        if (Global.getSettings().getModManager().isModEnabled("HMI")) {
            marketsToReplace.put(
                    HMI_ScrapMarketPlugin.class.getName(),
                    AoTDHMI_ScrapMarketPlugin.class.getName());
            marketsToReplace.put(
                    HMI_ExecMarketPlugin.class.getName(), AoTDHMI_ExecMarketPlugin.class.getName());
        }
        marketsToReplace.put(
                OpenMarketPlugin.class.getName(), AoTDOpenMarketPlugin.class.getName());
        marketsToReplace.put(
                BlackMarketPlugin.class.getName(), AoTDBlackMarketPlugin.class.getName());
        marketsToReplace.put(
                MilitarySubmarketPlugin.class.getName(),
                AoTDMilitarySubmarketPlugin.class.getName());
        marketsToReplace.put(
                LocalResourcesSubmarketPlugin.class.getName(),
                AoTDLocalResourcesSubmarketPlugin.class.getName());
    }
    ...
    public void onApplicationLoad() throws Exception {
        ...
        AoTDContractRewardCreatorManager.addCreator(
                GenericBlueprintCreator.class.getName(), new GenericBlueprintCreator());
        AoTDContractRewardCreatorManager.addCreator(
                GenericSpecialItemCreator.class.getName(), new GenericSpecialItemCreator());
        AoTDContractRewardCreatorManager.addCreator(
                GenericAICoreCreator.class.getName(), new GenericAICoreCreator());
        ...
        AoTDPlayerContractCreatorManager.addCreator(
                creatorAPI2.getBaseIdForContract(), creatorAPI2);

        StrategicArmamentSupply creatorAPI3 = new StrategicArmamentSupply();
        AoTDPlayerContractCreatorManager.addCreator(
                creatorAPI3.getBaseIdForContract(), creatorAPI3);

        CivilianSupplyProgram creatorAPI4 = new CivilianSupplyProgram();
        AoTDPlayerContractCreatorManager.addCreator(
                creatorAPI4.getBaseIdForContract(), creatorAPI4);
    }
    ...
    public void onAboutToStartGeneratingCodex() {
        ...
        if (Global.getSettings().getGameVersion().contains("0.98a")) {
            if (!Global.getSettings().getGameVersion().equals("0.98a-RC8")) {
                throw new RuntimeException(
                        "AoTD Theory of Toolbox: This version of mod for 0.98a game can only be run at exactly 0.98a-RC8 version ");
            }
            ...
            Industry first =
                    test.getIndustry(
                            getIndustryStringBasedOnOrder(
                                    0, Industries.HEAVYBATTERIES, Industries.REFINING));
            Industry second =
                    test.getIndustry(
                            getIndustryStringBasedOnOrder(
                                    1, Industries.HEAVYBATTERIES, Industries.REFINING));
        ...
        ReflectionUtilis.setPrivateVariableFromSuperclass(
                "economy", Global.getSector(), new AoTDEconomy(econ.isSimMode(), econ));
        loadMarketsToReplace();
        Global.getSettings()
                .getIndustrySpec(Industries.SPACEPORT)
                .addTag(AoTDIndTags.ALWAYS_ACTIVE_NON_PENDING);
        Global.getSettings()
                .getIndustrySpec(Industries.POPULATION)
                .addTag(AoTDIndTags.ALWAYS_ACTIVE_NON_PENDING);
        Global.getSettings()
                .getIndustrySpec(Industries.POPULATION)
                .setPluginClass(AoTDToolboxPopAndInfra.class.getName());
        for (AoTDCommodityEconSpec value : AoTDCommodityEconSpecManager.specs.values()) {
            if (value.econUnitMult != 1f) {
                ReflectionUtilis.invokeMethodWithAutoProjection(
                        "setEconUnit",
                        Global.getSettings().getCommoditySpec(value.commodityId),
                        Global.getSettings().getCommoditySpec(value.commodityId).getEconUnit()
                                * value.econUnitMult);
            }
        }
        ...
        for (SubmarketSpecAPI allSubmarketSpec : Global.getSettings().getAllSubmarketSpecs()) {
            String clas =
                    (String)
                            ReflectionUtilis.invokeMethodWithAutoProjection(
                                    "getScriptClass", allSubmarketSpec);
            if (marketsToReplace.containsKey(clas)) {
                ReflectionUtilis.replaceFirstStringFieldWithValue(
                        allSubmarketSpec, clas, marketsToReplace.get(clas), false);
            }
        }
        Global.getSettings()
                .getCommoditySpec(Commodities.SHIPS)
                .getTags()
                .add(AoTDTradeTags.AOTD_DOES_NOT_HAVE_EXCESS);
        Global.getSettings()
                .getCommoditySpec(Commodities.CREW)
                .getTags()
                .add(AoTDTradeTags.IGNORE_SCAVENGERS);
        Global.getSettings()
                .getCommoditySpec(Commodities.MARINES)
                .getTags()
                .add(AoTDTradeTags.IGNORE_SCAVENGERS);
    ...
    public void onCodexDataGenerated() {
        CodexEntryV2 plugin =
                (CodexEntryV2)
                        CodexDataV2.ROOT.getChildren().stream()
                                .filter(x -> x.getId().equals(CodexDataV2.CAT_INDUSTRIES))
                                .findAny()
                                .orElse(null);
        //        if(plugin!=null){
    ...
    public static void populateIndustries(CodexEntryPlugin parent) {
        ...
        for (final IndustrySpecAPI spec : specs) {
            ...
            CodexEntryV2 curr =
                    new AoTDToTIndustryEntryCodex(
                            getIndustryEntryId(spec.getId()),
                            spec.getName(),
                            spec.getImageName(),
                            spec) {
                        @Override
                        public void createTitleForList(
                                TooltipMakerAPI info, float width, ListMode mode) {
    ...
    public void onGameLoad(boolean newGame) {
        ...
        Global.getSector()
                .getListenerManager()
                .addListener(new AoTDGrandWonderDecivListener(), true);
        Global.getSector()
                .getListenerManager()
                .addListener(new AoTDCommodityTooltipInjector(), true);
        if (newGame) {
            for (MarketAPI marketAPI : Global.getSector().getEconomy().getMarketsCopy()) {
                marketAPI.reapplyConditions();
                for (Industry industry : marketAPI.getIndustries()) {
                    if (!AoTDIndustryData.getInstance(industry.getMarket())
                            .isPending(industry.getId())) {
                        industry.reapply();
                    }
                }
            }
        ...
        if (CommandTabMemoryManager.getInstance().getLastCheckedTab() != null) {
            if (CommandTabMemoryManager.getInstance()
                            .getLastCheckedTab()
                            .equalsIgnoreCase("economy")
                    || CommandTabMemoryManager.getInstance()
                            .getLastCheckedTab()
                            .equalsIgnoreCase("colonies")) {
                CommandTabMemoryManager.getInstance().setLastCheckedTab("domain");
            }
        }
        ...
        Global.getSector()
                .getListenerManager()
                .addListener(new AoTDToobloxIndustryListener(), true);
        Global.getSector()
                .addTransientScript(
                        new DelayedActionScript(0.1f) {
                            @Override
                            public void doAction() {
                                BarEventManager bar = BarEventManager.getInstance();
                                if (bar.hasEventCreator(DeliveryBarEventCreator.class)) {
                                    bar.getCreators()
                                            .removeIf(x -> x instanceof DeliveryBarEventCreator);
        ...
        Global.getSector()
                .getListenerManager()
                .removeListenerOfClass(StandardGroundRaidObjectivesCreator.class);
        Global.getSector()
                .getListenerManager()
                .addListener(new AoTDStandardGroundRaidObjectivesCreator(), true);
    }
