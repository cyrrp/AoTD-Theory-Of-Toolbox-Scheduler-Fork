...
public class AoTDToolboxFoodProd extends BaseMarketConditionPlugin {
    ...
    public void apply(String id) {
        boolean conditionStructureMutation =
                !market.hasCondition("aotd_toolbox_food_corrector_start")
                        || market.getConditions().get(market.getConditions().size() - 1)
                                != this.condition;
    ...
    private void applyWithoutConditionBoundary(String id) {
        ...
        for (Industry industry : market.getIndustries()) {
            if (industry.getSpec().hasTag(Industries.FARMING)
                    || industry.getSpec().hasTag("aquaculture")
                    || industry instanceof Farming) {
                ...
                industry.getSupply(Commodities.FOOD)
                        .getQuantity()
                        .modifyFlatAlways(prodId, toAdd, "AoTD corrector");
                industry.getDemand(Commodities.HEAVY_MACHINERY)
                        .getQuantity()
                        .modifyMultAlways(
                                prodId,
                                Math.round(0.3f * Math.pow(2, market.getSize() - 1)),
                                "AoTD corrector");
            }
        }
    }
    ...
    public int getSupplyFromIndustry(Industry industry, String comId) {
        ...
        for (MutableStat.StatMod object : stat.getFlatMods().values()) {
            ...
            } else if (object.getDesc() != null
                    && object.getDesc().contains("shortage")
                    && object.getDesc().contains("Heavy machinery")) {
                int totalDemanded =
                        industry.getDemand(Commodities.HEAVY_MACHINERY)
                                .getQuantity()
                                .getModifiedInt();
